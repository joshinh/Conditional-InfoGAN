...
#		del samples, labels
        ...
        samples,cond,pdf,labels = get_data(settings['data'], data_settings)
    ...
    return samples,cond,pdf,labels
...
def get_data(data_type, data_options=None):
    ...
    elif data_type == 'cond_waves':
        samples,cond = cond_wave(**data_options)
...
def get_cond_batch(samples, batch_size, batch_idx, labels=None):
    start_pos = batch_idx * batch_size
    end_pos = start_pos + batch_size
    if labels is None:
        return samples[start_pos:end_pos], None
...
def cond_wave(seq_length=30, num_samples=28*5*100, num_signals=1,
        freq_low=1, freq_high=5, amplitude_low = 0.1, amplitude_high=0.9, **kwargs):
    ix = np.arange(seq_length) + 1
    num_signals = 100
    samples = []
    conds = []
    alpha_list = np.arange(0.0,5.0,5.0/(num_samples//num_signals)) 
    for i in range(num_samples//num_signals):
        A = np.random.uniform(low=amplitude_high, high=amplitude_low)       # amplitude
        alpha = alpha_list[i]
        out = A*np.exp(-1*alpha*(ix/float(seq_length)))
        conds.append(out)
        signals = []
        for j in range(num_signals):
            ...
            # offset
            offset = np.random.uniform(low=-np.pi, high=np.pi)
            signals.append(np.stack((np.sin(2*np.pi*f*ix/float(seq_length) + offset)*out,out)))
            samples.append(np.array(signals).T)
            signals = []
    # the shape of the samples is num_samples x seq_length x num_signals
    samples = np.array(samples)
