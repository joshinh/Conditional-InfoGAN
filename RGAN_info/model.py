...
def sample_latent_C(batch_size, latent_dim, rand_type,lower_bound, upper_bound=-1):
    if rand_type == "continuous":
        assert upper_bound != -1
        assert latent_dim == 3
        c1 = np.random.uniform(0.1, 0.9, size=(batch_size, 1))
        c2 = np.random.uniform(1.0, 5.0, size=(batch_size, 1))
        c3 = np.random.uniform(-np.pi,np.pi, size=(batch_size,1))
        c = np.concatenate((c1,c2, c3),1)
    elif rand_type == "categorical":
        c = np.random.choice(lower_bound, size=(batch_size, latent_dim))
    else:
        print("Rand type not supported")
        exit()
    return c
...
def train_epoch(info, latent_C, latent_C_dim, epoch, samples, cond_samples, labels, sess, Z, X, CG, CD, CS, cond_sine, info_loss, D_loss, G_loss, info_solver, D_solver, G_solver, 
    ...
    for batch_idx in range(0, int(len(samples) / batch_size) - (D_rounds + (cond_dim > 0)*G_rounds), D_rounds + (cond_dim >0)*G_rounds):
        ...
        #update using info loss
        if info:
            Z_mb = sample_Z(batch_size, seq_length, latent_dim, use_time)
            C_mb = sample_latent_C(batch_size, latent_C_dim, "continuous", 0, 1)
            
            if cond_dim > 0:
                cond_mb = data_utils.get_cond_batch(cond_samples, batch_size, batch_idx, None)[0]
                _ = sess.run(info_solver, feed_dict={Z: Z_mb, latent_C: C_mb, cond_sine: cond_mb })
            else:
                _ = sess.run(info_solver, feed_dict={Z: Z_mb, latent_C: C_mb})

        # update the discriminator
        for d in range(D_rounds):
            X_mb, Y_mb = data_utils.get_batch(samples, batch_size, batch_idx + d, labels)
            if cond_dim > 0:
                cond_mb = data_utils.get_cond_batch(cond_samples, batch_size, batch_idx + d, None)[0]
            Z_mb = sample_Z(batch_size, seq_length, latent_dim, use_time)
            if info:
                C_mb = sample_latent_C(batch_size, latent_C_dim, "continuous", 0, 1)
            if cond_dim > 0:
                ...
                #     Y_wrong = 1 - Y_mb
                if info:
                    _ = sess.run(D_solver, feed_dict={X: X_mb, Z: Z_mb, latent_C: C_mb, cond_sine: cond_mb})
                else:
                    _ = sess.run(D_solver, feed_dict={X: X_mb, Z: Z_mb, cond_sine: cond_mb})
            else:
                if info:
                    _ = sess.run(D_solver, feed_dict={X: X_mb, Z: Z_mb, latent_C: C_mb})
                else:
        ...
        for g in range(G_rounds):
            if cond_dim > 0:
                ...
                cond_mb = data_utils.get_cond_batch(cond_samples, batch_size, batch_idx + D_rounds + g, None)[0]
                if info:
                    _ = sess.run(G_solver,
                        feed_dict={Z: sample_Z(batch_size, seq_length, latent_dim, use_time=use_time), cond_sine: cond_mb,
                        latent_C: sample_latent_C(batch_size, latent_C_dim, "continuous", 0, 1)})
                else:
                    _ = sess.run(G_solver,
                        feed_dict={Z: sample_Z(batch_size, seq_length, latent_dim, use_time=use_time), cond_sine: cond_mb})
            else:
    ...
    if cond_dim > 0:
        if info:
            info_loss_curr, D_loss_curr, G_loss_curr = sess.run([info_loss, D_loss, G_loss], feed_dict={X: X_mb, Z: sample_Z(batch_size, seq_length, latent_dim, use_time=use_time),
                latent_C: sample_latent_C(batch_size, latent_C_dim, "continuous", 0, 1), cond_sine: cond_mb})
        else:
            D_loss_curr, G_loss_curr = sess.run([D_loss, G_loss], feed_dict={X: X_mb, Z: sample_Z(batch_size, seq_length, latent_dim, use_time=use_time), cond_sine: cond_mb})
        ...
        G_loss_curr = np.mean(G_loss_curr)
    if info:
        return info_loss_curr, D_loss_curr, G_loss_curr
    else:
...
def WGAN_loss(Z, X, WGAN_clip=False):
    ...
    D_real, D_logit_real, D_logit_real_final = discriminator(False,1, X, hidden_units_d, seq_length, batch_size)
...
#        gradient_penalty = tf.reduce_mean((slopes-1)**2)
        ...
        D_offset, _ = discriminator(False,1, X + h, hidden_units_d)
...
def GAN_loss(info, latent_C, latent_C_dim, Z, X, generator_settings, discriminator_settings, kappa, cond, CG, CD, CS, cond_sine, wrong_labels=False):
    if cond:
        # C-GAN
        G_sample = generator(info, latent_C, Z, **generator_settings, c=cond_sine)
        if info:
            info_logits_real, D_real, D_logit_real =  discriminator(info, latent_C_dim, X, **discriminator_settings, c=cond_sine)
            info_logits_fake, D_fake, D_logit_fake = discriminator(info, latent_C_dim, G_sample, reuse=True, **discriminator_settings, c=cond_sine)
        else:
            D_real, D_logit_real =  discriminator(info, latent_C_dim, X, **discriminator_settings, c=cond_sine)
            D_fake, D_logit_fake = discriminator(info, latent_C_dim, G_sample, reuse=True, **discriminator_settings, c=cond_sine)
        
        if wrong_labels:
            # the discriminator must distinguish between real data with fake labels and real data with real labels, too
            D_wrong, D_logit_wrong = discriminator(info, latent_C_dim, X, reuse=True, **discriminator_settings, c=CS)
    else:
        # normal GAN
        G_sample = generator(info, latent_C, Z, **generator_settings)
        if info:
            info_logits_real, D_real, D_logit_real  = discriminator(info, latent_C_dim, X, **discriminator_settings)
            info_logits_fake, D_fake, D_logit_fake = discriminator(info, latent_C_dim, G_sample, reuse=True, **discriminator_settings)
        else:
            D_real, D_logit_real  = discriminator(info, latent_C_dim, X, **discriminator_settings)
            D_fake, D_logit_fake = discriminator(info, latent_C_dim, G_sample, reuse=True, **discriminator_settings)
    ...
    if info:
        info_loss_real = tf.losses.mean_squared_error(labels=latent_C, predictions=info_logits_real)
        info_loss_fake = tf.losses.mean_squared_error(labels=latent_C, predictions=info_logits_fake)
        info_loss = info_loss_fake
    ...
    if info:
        return info_loss, D_loss, G_loss
    else:
        return D_loss, G_loss

def GAN_solvers(info, info_loss, D_loss, G_loss, learning_rate, batch_size, total_examples, 
    ...
    if info:
        info_solver = tf.train.GradientDescentOptimizer(learning_rate=learning_rate/100).minimize(info_loss, var_list=discriminator_vars+generator_vars)
    ...
    if info:
        return info_solver, D_solver, G_solver, priv_accountant
    else:
        return D_solver, G_solver, priv_accountant

# --- to do with the model --- #

def create_placeholders(batch_size, seq_length, latent_dim, num_generated_features, cond_dim, info, latent_C_dim=None):
    ...
    cond_sine = tf.placeholder(tf.float32, [batch_size, seq_length, cond_dim])
    if info:
        assert latent_C_dim is not None
        latent_C = tf.placeholder(tf.float32, [batch_size, latent_C_dim])
        return latent_C, Z, X, CG, CD, CS, cond_sine
    else:
        return Z, X, CG, CD, CS, cond_sine

def generator(info, latent_C, z, hidden_units_g, seq_length, batch_size, num_generated_features, reuse=False, parameters=None, cond_dim=0, c=None, learn_scale=True):
    """
    ...
    with tf.variable_scope("generator") as scope:
        ...
        #New code
        if info:
            repeated_latent_c = tf.stack([latent_C]*seq_length, axis=1)
            z = tf.concat([z, repeated_latent_c], axis=2)
...
def discriminator(info, latent_C_dim, x, hidden_units_d, seq_length, batch_size, reuse=False, 
    ...
    with tf.variable_scope("discriminator") as scope:
        ...
        b_out_D = tf.get_variable(name='b_out_D', shape=1,
                initializer=tf.truncated_normal_initializer())
        if info:
            W_out_info = tf.get_variable(name='W_out_info', shape=[hidden_units_d, latent_C_dim],
                initializer=tf.truncated_normal_initializer())
            b_out_info = tf.get_variable('b_out_info', shape=latent_C_dim,
                initializer=tf.truncated_normal_initializer())
...
#        logit_final = tf.matmul(rnn_outputs[:, -1], W_final_D) + b_final_D
        ...
        if info:
            info_logits = tf.einsum('ijk,km', rnn_outputs, W_out_info) + b_out_info
            info_logits = tf.reduce_mean(info_logits, axis=1)
...
#        logits = tf.matmul(rnn_outputs_flat, W_out_D) + b_out_D
    ...
    if info:
        return info_logits, output, logits
    else:
...
def sample_trained_model(settings, epoch, num_samples, Z_samples=None, C_samples=None):
    ...
    if settings['info']:
        latent_C = tf.placeholder(tf.float32, [num_samples, settings['latent_C_dim']])
    else:
        latent_C = None
    CG = tf.placeholder(tf.float32, [num_samples, settings['cond_dim']])
    cond_sine = tf.placeholder(tf.float32, [num_samples, seq_length, cond_dim])
    ...
    if settings['info']:
        latent_C_samples = sample_latent_C(num_samples, settings['latent_C_dim'], "continuous", 0, 1)

    if C_samples is None:
        # normal GAN
        G_samples = generator(settings['info'], latent_C, Z, settings['hidden_units_g'], settings['seq_length'], 
        ...
        G_samples = generator(settings['info'], latent_C, Z, settings['hidden_units_g'], settings['seq_length'], 
                              num_samples, settings['num_generated_features'], 
                              reuse=False, parameters=parameters, cond_dim=settings['cond_dim'], c=cond_sine)
    # sample from it 
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        if settings['info']:
            if C_samples is None:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples, latent_C: latent_C_samples })
            else:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples, CG: C_samples, latent_C: latent_C_samples, cond_sine: C_samples})
        else:
            if C_samples is None:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples})
            else:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples, CG: C_samples, cond_sine: C_samples})
    tf.reset_default_graph()
    return real_samples


def sample_trained_model_varying(settings, epoch, num_samples, Z_samples=None, C_samples=None):
    """
    Return num_samples samples from a trained model described by settings dict
    """
    # if settings is a string, assume it's an identifier and load
    ...
    if settings['info']:
        ...
        latent_C = None
    CG = tf.placeholder(tf.float32, [num_samples, settings['cond_dim']])
    if settings['cond_dim'] > 0:
        cond_sine = tf.placeholder(tf.float32, [num_samples, settings['seq_length'], settings['cond_dim']])
    if Z_samples is None:
        Z_samples = sample_Z(num_samples, settings['seq_length'], settings['latent_dim'], settings['use_time'], use_noisy_time=False)
    else:
        assert Z_samples.shape[0] == num_samples
    # create the generator (GAN or CGAN)
    if settings['info']:
        latent_C_samples = np.expand_dims(np.arange(-np.pi,np.pi,2*np.pi/num_samples),1)
        latent_C_samples = np.concatenate((0.9 * np.ones((num_samples, 1), np.float32), 
            5.0 * np.ones((num_samples, 1), np.float32), latent_C_samples), -1)
        #latent_C_samples = np.tile(np.expand_dims(latent_C_samples,1), (1,settings['latent_C_dim']))
    ...
    with tf.Session() as sess:
        ...
        if settings['info']:
            if C_samples is None:
                ...
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples, latent_C: latent_C_samples, cond_sine: C_samples})
        else:
            if C_samples is None:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples})
            else:
                real_samples = sess.run(G_samples, feed_dict={Z: Z_samples, cond_sine: C_samples})
    tf.reset_default_graph()
    return real_samples
