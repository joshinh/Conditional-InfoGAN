...
def sine_plot(identifier, epoch):

    n_samples = 20
    settings_sine = json.load(open('./experiments/settings/' + identifier + '.txt', 'r'))
    data_sine = np.load('./experiments/data/' + identifier + '.data.npy').item()
    if settings_sine['cond_dim'] > 0:
        cond_sine = np.load('./experiments/cond_data/' + identifier + '.data.npy').item()
        cond_samples = data_utils.get_cond_batch(cond_sine['cond_samples'], n_samples, 0, None)[0]
        samples_sine = model.sample_trained_model_varying(settings_sine, epoch, n_samples, C_samples=cond_samples)
    else:
        samples_sine = model.sample_trained_model_varying(settings_sine, epoch, n_samples)

    plotting.save_plot_sample_varying(samples_sine, epoch, identifier, n_samples,
                num_epochs=epoch, ncol=4)
    if settings_sine['cond_dim'] > 0:
        plotting.save_plot_sample_varying(cond_samples, epoch, identifier + "_base", n_samples, num_epochs=epoch, ncol=4)
